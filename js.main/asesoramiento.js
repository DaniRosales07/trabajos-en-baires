...
let cardContainer4 = document.getElementById('cardContainer4');

// Cargar el archivo JSON
fetch('../db/db.json')
    .then(response => response.json())
    .then(data => {
        // Obtener los datos de la página 4
        let datosPagina4 = data.pagina4;

        // Crear las cards dinámicamente con los datos de la página 4
        datosPagina4.forEach(item => {
            let nuevaCard = createCard(item.servicio, item.descripcion, item.precio, item.imagen);
            addCardToContainer(cardContainer4, nuevaCard);
        });
    })
...
function finalizarCompra() {
    if (cart.length === 0) {
        ...
        let mensaje = 'Hola buen día, vengo del sitio Web de Trabajos en Buenos Aires. Me interesan los siguientes productos:' + ' ';
