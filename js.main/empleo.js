...
fetch('../db/db.json')
    ...
    .then(data => {
        // Obtener los datos de la página 1
        let datosPagina2 = data.pagina2;

        // Crear las cards dinámicamente con los datos de la página 1
        datosPagina2.forEach(item => {
            let nuevaCard = createCard(item.servicio, item.descripcion, item.precio, item.imagen);
            addCardToContainer(cardContainer2, nuevaCard);
        });
    })
